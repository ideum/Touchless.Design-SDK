...
using TouchlessDesign.Core;
using Timer = System.Threading.Timer;

namespace TouchlessDesign.Components.Input {

  public class Input : AppComponent, ICursor {
    ...
    private void InitializeInputProvider() {
      ...
      var path = Path.Combine(DataDir, ProvidersRootDir, _settings.InputProviderPath);
      try {
        if (!File.Exists(path)) {
    ...
    public class InputSettings {
      ...
      public static InputSettings Defaults() {
        return new InputSettings {
          ...
          ToggleEmulationToggleSpeed_ms = 1500d,
