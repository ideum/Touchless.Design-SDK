...
Usage: Send strings to the serial port:
...
"brightness" - sets the max brightness of the strip
...
void loop() {
  ...
  while (Serial.available() > 0) {
    ...
    if (inString.startsWith("#")) 
    {
      ...
      Serial.printf("%d\n",b);
      colorFade(strip.Color(r,g,b),10,pedestal);
    }
...
void colorFade(uint32_t color, int t, bool pedestal) 
{
  for (int fadeValue = 0 ; fadeValue <= maxBrightness; fadeValue += 5) 
  {
    if (pedestal) {
