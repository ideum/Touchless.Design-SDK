...
public class Client : IDisposable, Parser.IListener, Connection.IListener
  {
    ...
    public void Receive(byte[] bytes)
    {
      ...
      {
        UnityEngine.Debug.LogError($"Parse error when deserializing msg received from '{Destination}' of {raw}: {e}");
      }
    }
    ...
    public void OnMessage(byte[] data)
    {
      if (data == null)
      {
        Debug.Log("No bytes are written");
        Receive(null);
        return;
      }
      if (Connection == null)
      {
        Debug.Log("Connection or Socket is null for payload callback");
        return;
      }
      Receive(data);
    }
